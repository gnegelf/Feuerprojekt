function [ A,b_U,b_L,c,solInhom,solBasis,Aext,b_Uext,b_Lext] = constrMIPEliminationRobinVertex(i1,i2,i3, R,xn,tn,dx,dt,N,u,contPos,a1,a2,step,C,g,f,Tmax,Schwell,hL,hR,hU,hO,D)
...
global usetime;
for t=2:2+floor(usetime/dt) %the finer the grid the more indices are set in the basis
    for i=2:xn+2
        for j=2:xn+2
            for k=1:N
                if abs(u((i-2)*dx,(j-2)*dx,contPos(k,1),contPos(k,2)))>0.0001
                    contBasis((t-1)*xn1^2+(i-1)*xn1+j,(2-2)*N+k)=u((i-2)*dx,(j-2)*dx,contPos(k,1),contPos(k,2));
                    %the water has an immediate impact consider shifting
                    %one more timestep
                end
            end
        end
        ...
        for j=1:N+1
            ...
            if (i>1) && (j>1)%the input from source is independent of the heat flow
                ...
                for cc=0:C(i,j,2)-1 %additional flow that might still be on the arc from earlier timeframes
                    if t>C(i,j,2)*step
                        for pp=0:step-1
                            condM((t-1)*N1^2+(i-1)*N1+j,i2+(t-1-pp-cc*step)*N1^2+(i-1)*N1+j)=1;
                        end
                    end
                end
            end
