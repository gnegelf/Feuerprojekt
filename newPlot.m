...
if video
    ...
    for i=1:N
       for k=1:N
           if C(i+1,k+1,1)>10^(-5)
               length=sqrt((Graph(i,1)-Graph(k,1))*(Graph(i,1)-Graph(k,1))+(Graph(i,2)-Graph(k,2))*(Graph(i,2)-Graph(k,2)));
               if length < minlength
                  minlength=length; 
               end
           end
                        ...
                        for co =0:step-1
                            if j>co+cc*step
                                if flowMatrix(i,k,j-cc*step-co)>10^(-5)
                                    fact=minlength/sqrt((Graph(i,1)-Graph(k,1))*(Graph(i,1)-Graph(k,1))+(Graph(i,2)-Graph(k,2))*(Graph(i,2)-Graph(k,2)));
                                    ...
                                    x2=Graph(i,1)+(Graph(k,1)-Graph(i,1))*((ii-1+fact+cc*slow*step+co*slow)/(slow*step*C(i+1,k+1,2)));
                                    y2=Graph(i,2)+(Graph(k,2)-Graph(i,2))*((ii-1+fact+cc*slow*step+co*slow)/(slow*step*C(i+1,k+1,2)));
