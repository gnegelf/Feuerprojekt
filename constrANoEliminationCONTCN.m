function [ A,b_U,b_L,c ] = constrANoEliminationCONT(i1,i2,i3, xn,tn,dx,dt,N,u,contPos,a1,a2,step,C,g,Tmax,Schwell,hL,hR,hU,hO,D)
...
if ~CN
    ...
    for i=2:xn+2
        for j=2:xn+2
            ...
            ry=dt/dx*a2;
            %difM((i-1)*xn1+j,(i-1)*xn1+j+xn1^2  )=1+2*R((i-2)*dx,(j-2)*dx);
            difM((i-1)*xn1+j,(i-1)*xn1+j+xn1^2  )=4*(1+2*R);
            difM((i-1)*xn1+j,(i-2)*xn1+j+xn1^2  )=-2*R-rx; 
            difM((i-1)*xn1+j,(i-1)*xn1+j-1+xn1^2)=-2*R-ry;
            difM((i-1)*xn1+j,(i-1)*xn1+j+1+xn1^2)=-2*R+ry;
            difM((i-1)*xn1+j, i   *xn1+j+xn1^2  )=-2*R+rx;
            %difM((i-1)*xn1+j,(i-1)*xn1+j  )=-1+2*R((i-2)*dx,(j-2)*dx);
            difM((i-1)*xn1+j,(i-1)*xn1+j ) =-4*(1-2*R);
            difM((i-1)*xn1+j,(i-2)*xn1+j ) =-2*R-rx; 
            difM((i-1)*xn1+j,(i-1)*xn1+j-1)=-2*R-ry;
            difM((i-1)*xn1+j,(i-1)*xn1+j+1)=-2*R+ry;
            difM((i-1)*xn1+j, i   *xn1+j  )=-2*R+rx;
        end
    end
end
...
[m,n]=size(A);
c=zeros(n,1);
factors=dt*dx*dx*ones(tn1*(xn+3)^2,1);
for t=1:tn1
    if (t==1 || t==tn1)
       fact=0.5; 
       for i=3:xn+1
            for j=2:xn+2
                if (j==2 || j==xn+2)
                   factors((t-1)*(xn+1)^2+(i-1)*(xn+1)+j)=dt*dx*dx*0.5*fact; 
                    ...
                    factors((t-1)*(xn+1)^2+(i-1)*(xn+1)+j)=dt*dx*dx*fact;
                    factors((t-1)*(xn+1)^2+(j-1)*(xn+1)+i)=dt*dx*dx*fact;
                end
            end
        end
    else
...
c(n-i3-i2-i2+1:n)=-0.00001*ones(i3+i2+i2,1);
end