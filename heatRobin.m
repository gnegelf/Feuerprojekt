...
%
    ...
    if ~all(isfield(coeffs,{'v','h'}))
        ...
        if isscalar(coeffs.h)
            coeffs.h=coeffs.h*ones(4,1);
        elseif numel(coeffs.T_a)~=4
             error('heatRobin:argchk','Input argument ''coeffs.h'' need to be a scalar or a vector of length 4');
        end
...
%     sol    = ode15s(@(t,x) (t<=0.5)*b_f(DOFs)+b_r(DOFs)-A*x,tpts,T0(DOFs),odeopt);
    sol    = ode15s(@(t,x) b_f(DOFs)*(t<=usetime)+b_r(DOFs)-A*x,tpts,T0(DOFs),odeopt);
%     sol    = ode23s(@(t,x) b(DOFs)-A*x,tpts,T0(DOFs),odeopt);
    ...
    [i,tree,l] = mesh.point_location(xpts);
    %[i,l]  = pointLocation(TR,xpts);
    if find(isnan(i))
       save('bugDump','xpts','tpts','f','T0','varargin'); 
       find(isnan(i))
       xpts(find(isnan(i)),:)
    end
    clear TR;
