function [T,intT,mesh] = heatRobin(xpts,tpts,f,T0,varargin)
...
%     sol    = ode15s(@(t,x) (t<=0.5)*b_f(DOFs)+b_r(DOFs)-A*x,tpts,T0(DOFs),odeopt);
    sol    = ode15s(@(t,x) b_f(DOFs)*(t<tpts(2))+b_r(DOFs)-A*x,tpts,T0(DOFs),odeopt);
%     sol    = ode23s(@(t,x) b(DOFs)-A*x,tpts,T0(DOFs),odeopt);
    ...
    intR = detD'*Tmid;
    intT = (intR(1:end-1)+intR(2:end))./2.*diff(tpts);
